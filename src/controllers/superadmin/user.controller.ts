...
const isValidRole = (val: any): val is UserRole => {
  return ["superadmin", "operator", "customer"].includes(String(val));
};
...
export const getUsers = async (req: Request, res: Response) => {
  try {
    ...
    if (req.query.role && isValidRole(req.query.role)) {
...
export const createUser = async (req: Request, res: Response) => {
  const { username, email, role, company_id, latitude, longitude, password } = req.body;
...
export const updateUser = async (req: Request, res: Response) => {
  const id = String(req.params.id);
  if (!id) return res.status(400).json({ message: "Invalid user id" });
  const { username, email, role, company_id, latitude, longitude } = req.body;
  try {
    const data: any = {
      username,
      email,
